...
def main():
    ...
    for child in newer_root.getchildren():
        if child.tag == 'project':
            ...
            #FIND_PATTERN='./project[@name='+"'"+prj_path+"'"+']'   --pretty=format:"%s"
            ...
            if old_match is None:
                ...
                while count < int(commit_count):
                     COMMAND="git log --pretty=format:\"%s\" "+newer_prj_revision+"~"+str(count+1)+".."+newer_prj_revision+"~"+str(count)
                     ...
                     COMMAND="git log --pretty=format:\"%s\""+newer_prj_revision+"~"+str(count+1)+".."+newer_prj_revision+"~"+str(count)+unicode(" | grep '作者'")
                     output=os.popen(COMMAND).read()
                     print "command:"+COMMAND+"\n"
                     print "output:{}\n".format(output)
                     if  output:
                         ...
                         COMMAND="git log --pretty=format:\"%s\" "+newer_prj_revision+"~"+str(count+1)+".."+newer_prj_revision+"~"+str(count)+" | grep 'Author'"
                         output=os.popen(COMMAND).read()

...
function copy_debug_images()
{
  ...
  for f in `cat debug_spreadtrum.list`;
  do
    echo $f
    dirPath=`dirname $f`
    echo $dirPath
...
# 复制modem等bin文件
cp -rf $PROJECT_CODE_ROOT/device/sprd/modem_bin/* $PROJECT_CODE_ROOT/out/target/product/sp7731e_1h20

#打包pac文件
cd $PROJECT_CODE_ROOT/out/target/product/sp7731e_1h20
bash -x ./pac.sh
...
function copy_release_images()
{
  ...
  for f in `cat release_spreadtrum.list`;
  do
    echo $f
    #dirPath = `dirname $f`
    #if [ ! -d $dirPath ]; then

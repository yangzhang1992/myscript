...
generate_static_xml()
{
    ...
    if [ -d ${manifest_static}/spreadtrum/Snapshot_XML/${PROJECT_BRANCH_NAME} ];then
        echo "${manifest_static}/spreadtrum/Snapshot_XML/${PROJECT_BRANCH_NAME} exist"
    else
        echo "create ${manifest_static}/spreadtrum/Snapshot_XML/${PROJECT_BRANCH_NAME}"
        mkdir -p ${manifest_static}/spreadtrum/Snapshot_XML/${PROJECT_BRANCH_NAME}
    fi

    NEW_TAG=${PROJECT_BRANCH_NAME}_${CURRENT_DATETIME}.xml
    ${PROJECT_CODE_ROOT}/.repo/repo/repo manifest -o ${NEW_TAG} -r
    echo "cp ${NEW_TAG} ${manifest_static}/spreadtrum/Snapshot_XML/${PROJECT_BRANCH_NAME}"
    cp -rf ${NEW_TAG} ${manifest_static}/spreadtrum/Snapshot_XML/${PROJECT_BRANCH_NAME}
    ...
    BUILD_FLAG=1
        ...
        cp ${manifest_static}/spreadtrum/Snapshot_XML/${PROJECT_BRANCH_NAME}/$OLD_TAG ./
